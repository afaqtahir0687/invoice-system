            ...
            <div class="card">
                <div class="card-body">
                    <table class="table table-borderless mb-0">
                        <tr><td>Subtotal</td><td><input type="number" step="0.01" name="subtotal" id="subtotal" class="form-control form-control-sm text-end border-0" readonly value="{{ $invoice->subtotal }}"></td></tr>
                        <tr><td>Total Tax</td><td><input type="number" step="0.01" name="total_tax" id="total_tax" class="form-control form-control-sm text-end border-0" readonly value="{{ $invoice->total_tax }}"></td></tr>
                        <tr><td>Total Discount</td><td><input type="number" step="0.01" name="total_discount" id="total_discount" class="form-control form-control-sm text-end border-0" readonly value="{{ $invoice->total_discount }}"></td></tr>
                        <tr><td>Shipping</td><td><input type="number" step="0.01" name="shipping" id="shipping" class="form-control form-control-sm text-end" value="{{ $invoice->shipping }}"></td></tr>
                        <tr class="table-light fw-bold"><td>Grand Total</td><td><input type="number" step="0.01" name="grand_total" id="grand_total" class="form-control form-control-sm text-end border-0 fw-bold" readonly value="{{ $invoice->grand_total }}"></td></tr>
                    </table>
                </div>
            </div>
            <div class="text-end mt-4">
                <a href="{{ route('invoices.index') }}" class="btn btn-secondary me-2">Cancel</a>
                <button type="submit" class="btn btn-lg btn-success px-5">Update Invoice</button>
            </div>
        </div>
    </div>
</form>
...
<script>
    ...
    function addRow() {
        ...
        itemRows.appendChild(tr); calculate();
    }
    function calculate() {
